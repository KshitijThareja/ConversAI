...
export function SearchModal({ chats, currentChatId, onChatSelect, onNewChat, trigger }: SearchModalProps) {
  ...
  return (
    <Dialog open={searchOpen} onOpenChange={setSearchOpen}>
      ...
      <DialogContent className="sm:max-w-[425px] md:max-w-[600px] md:h-[450px] border-2 bg-gray-50 dark:bg-[#2f2f2f]">
        <div className="grid gap-4 py-4">
          <Input
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Type to search chats..."
            className="w-full bg-transparent border-none text-lg h-12 pl-0 placeholder:text-gray-400 focus:outline-none focus:border-none focus:ring-0 focus:ring-offset-0 border-2 border-gray-200 dark:border-gray-700 rounded-lg"
          />
